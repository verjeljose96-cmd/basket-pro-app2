...
st.title("Basket Pro") st.write("Prediccion NBA con Monte Carlo")
...
spread_line = st.number_input("Spread del mercado", value=-4.5) odds_home = st.number_input("Cuota local", min_value=1.01, value=1.90) odds_away = st.number_input("Cuota visitante", min_value=1.01, value=1.90)

SIMS = 10000

if st.button("Calcular"): if home == away: st.error("Selecciona equipos distintos") st.stop()
